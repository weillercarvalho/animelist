...
server.get(`/animeslist`, async (req: Request, res: Response) => {
  ...
  try {
    const gettingdateSection = await connection.query(`SELECT * FROM sessions ORDER BY id DESC LIMIT 1;`)
    const getuserId:number = gettingdateSection.rows[0].userId;
    const gettoken:string = gettingdateSection.rows[0].token;
    if (gettoken !== token) {
        return res.sendStatus(STATUS_CODE.NOT_FOUND);
    }
    const query = await connection.query(`SELECT * FROM animes WHERE "userId" = $1`,[getuserId]);
...
server.post(`/animeslist`, async (req: Request, res: Response) => {
  ...
  if (!name || !review || !image || rate) {
    return res.sendStatus(STATUS_CODE.UNAUTHORIZED); //PROBLEMA 401 NESSA LINHA TENHO QUE RESOLVER.
  }
    console.log(req.body)
  try {
    const gettingSession = await connection.query(`SELECT * FROM sessions ORDER BY id DESC LIMIT 1;`);
    ...
    const query = await connection.query(`INSERT INTO animes ("userId", "sessionId", "name", "review", "image", "rate") VALUES ($1,$2,$3,$4,$5,$6)`,[getuserId, getsessionId, name, review, image, rate]);
    return res.sendStatus(STATUS_CODE.CREATED);
  } catch (error) {
    return res.status(STATUS_CODE.SERVER_ERROR).send(error.message);
  }
